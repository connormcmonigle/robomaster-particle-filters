...
from robomaster_particle_filters import plate_orbit


ANGULAR_VELOCITY_STEP_SIZE = 2.0
POSITION_DIAGONAL_COVARIANCE = np.array([0.0001, 0.0001, 0.0001])
RADIAL_OFFSETS = [0.03, 0.0, 0.03, 0.0]
Z_OFFSETS = [0.0, 0.0, 0.0, 0.0]
...
class InteractivePlateOrbitFilterDemo:
    ...
    particle_filter_configuration: plate_orbit.ParticleFilterConfigurationParameters
    input_image: npt.NDArray[np.uint8]
    filter_lock: Lock
    latest_update_time: Optional[float]
    filter: Optional[plate_orbit.ParticleFilter]

    def __init__(self, window_title: str, resolution: Tuple[int, int], simulated_robot_state: SimulatedRobotState, number_of_particles: int, config: plate_orbit.ParticleFilterConfigurationParameters):
    ...
    def render_loop(self):
        ...
        while True:
            if self.filter is not None and self.latest_update_time is not None:
                ...
                x, y, _ = prediction.center()
                v_x, v_y, _ = prediction.center_velocity()
                center = int(self.w * x), int(self.h * y)
                end = int(self.w * (x + 0.5 * v_x)
                          ), int(self.h * (y + 0.5 * v_y))
                radii = int(self.w * prediction.radius_0()
                            ), int(self.h * prediction.radius_1())
...
if __name__ == "__main__":
    config = plate_orbit.ParticleFilterConfigurationParameters(
        0.11, 2.0, 0.001, 0.005, 0.0001, 6.0, 0.8, np.array([6.0, 6.0, 0.01]), np.array([3.0, 3.0, 0.001]))
    simulated_robot_state = SimulatedRobotState(
        radius=0.1, angle=0.0, angular_velocity=0.0, center=np.zeros(3), observer=np.zeros(3))
    demo = InteractivePlateOrbitFilterDemo(window_title="demo", resolution=(
